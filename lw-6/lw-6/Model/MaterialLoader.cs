...
public class MaterialLoader
{
    ...
    public int GetTextureId(string filePath)
    {
        ...
        {
            System.Console.WriteLine($"Texture file not found: {filePath}");
        ...
        if (material.GetMaterialTextureCount(TextureType.Diffuse) > 0)
        {
            ...
            else
            {
                System.Console.WriteLine(
                    $"Warning: Material {materialIndex} reports diffuse textures but slot 0 is unavailable.");
            }
    ...
    {
        if (materialIndexToTextureId.TryGetValue(materialIndex, out int textureId) && textureId != 0)
        {
        ...
        GL.Material(MaterialFace.Front, MaterialParameter.Diffuse,
            new float[] { diffuse.R, diffuse.G, diffuse.B, diffuse.A });
    }
